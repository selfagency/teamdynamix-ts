...
describe('SDK curated mutator coverage', () => {
  it('executes curated mutators across all domains', async () => {
    ...
    expect(await client.tickets.updateTicket({ appId: 1, ticketId: 101, body: { Title: 'y' } })).toEqual({
      id: 101,
      updated: true,
    });
    expect(await client.tickets.addTicketComment({ appId: 1, ticketId: 101, body: { Body: 'note' } })).toEqual({
      id: 1,
    });

    expect(
      await client.ticketRelationships.createTicketTask({ appId: 1, ticketId: 101, body: { Name: 'task' } }),
    ).toEqual({ id: 201 });
    expect(await client.ticketRelationships.addTicketAsset({ appId: 1, ticketId: 101, assetId: 301 })).toEqual({
      id: 301,
    });
    expect(
      await client.ticketRelationships.removeTicketAsset({ appId: 1, ticketId: 101, assetId: 301, confirm: true }),
    ).toEqual({});
    expect(
      await client.ticketRelationships.addTicketContact({ appId: 1, ticketId: 101, contactUid: 'user-1' }),
    ).toEqual({ id: 'user-1' });
    expect(
    ...
    expect(await client.knowledgeBase.createArticle({ appId: 1, body: { Subject: 'kb' } })).toEqual({ id: 401 });
    expect(await client.knowledgeBase.updateArticle({ appId: 1, articleId: 401, body: { Subject: 'kb2' } })).toEqual({
      id: 401,
      updated: true,
    });

    expect(await client.projects.createIssue({ body: { Title: 'i' } })).toEqual({ id: 501 });
    expect(await client.projects.createRisk({ body: { Title: 'r' } })).toEqual({ id: 601 });

    expect(await client.services.createService({ appId: 1, body: { Name: 'svc' } })).toEqual({ id: 701 });
    expect(await client.services.updateService({ appId: 1, serviceId: 701, body: { Name: 'svc2' } })).toEqual({
      id: 701,
      updated: true,
    });
    expect(await client.services.deleteService({ appId: 1, serviceId: 701, confirm: true })).toEqual({});
    expect(await client.services.createServiceCategory({ appId: 1, body: { Name: 'cat' } })).toEqual({ id: 702 });
    expect(await client.services.updateServiceCategory({ appId: 1, categoryId: 702, body: { Name: 'cat2' } })).toEqual({
      id: 702,
      updated: true,
    });
    expect(await client.services.deleteServiceCategory({ appId: 1, categoryId: 702, confirm: true })).toEqual({});

    expect(await client.assets.createAsset({ appId: 1, body: { Name: 'asset' } })).toEqual({ id: 801 });
    expect(await client.assets.updateAsset({ appId: 1, assetId: 801, body: { Name: 'asset2' } })).toEqual({
      id: 801,
      updated: true,
    });
    expect(await client.assets.deleteAsset({ appId: 1, assetId: 801, confirm: true })).toEqual({});

    expect(await client.cmdb.createConfigurationItem({ appId: 1, body: { Name: 'ci' } })).toEqual({ id: 901 });
    expect(
      await client.cmdb.updateConfigurationItem({ appId: 1, configurationItemId: 901, body: { Name: 'ci2' } }),
    ).toEqual({ id: 901, updated: true });
    expect(await client.cmdb.deleteConfigurationItem({ appId: 1, configurationItemId: 901, confirm: true })).toEqual(
      {},
    );

    expect(await client.time.createTimeEntry({ body: { Minutes: 30 } })).toEqual({ id: 1001 });
    expect(await client.time.updateTimeEntry({ timeEntryId: 1001, body: { Minutes: 60 } })).toEqual({
      id: 1001,
      updated: true,
    });
